...
while True:
    ...
    if chosenAlgo == 1:
        ...
        for i in range(agentsc):
            for pillar in pillars:
                if (
                    agents[i].dead == False
                    and birds[i].rect.colliderect(pillar.upperPillar)
                    or birds[i].rect.colliderect(pillar.lowerPillar)
                ):
                ...
                while len(qualities) != 0:
                    ...
                    if chosenQuality in qualities:
                        ...
                        try:
                            ...
                            if chosenQuality == 1:
                                agents[i].velocityLimitLow = survivors[
                                    pair[whichParent]
                                ].velocityLimitLow
                            elif chosenQuality == 2:
                                agents[i].velocityLimitUp = survivors[
                                    pair[whichParent]
                                ].velocityLimitUp
        ...
        collide = False
        for pillar in pillars:
            if birds[0].rect.colliderect(pillar.upperPillar) or birds[
                0
            ].rect.colliderect(pillar.lowerPillar):
                collide = True
    ...
    if chosenAlgo == 1:
        ...
        datasurface = algofont.render(
            "Generation: " + str(currentGeneration), False, (0, 0, 0)
        )
        screen.blit(datasurface, (750, 50))
        datasurface = algofont.render(
            "Alive: " + str(agentsc - deadcount) + "/" + str(agentsc), False, (0, 0, 0)
        )
