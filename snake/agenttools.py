...
def bfsTarget(g, s, path, target):
    ...
    if found:
        ...
        print(
            f"New path: target = {target.xcoord},{target.ycoord}, current = {s.xcoord},{s.ycoord}"
        )
...
def longestPath(g, s):
    ...
    while 1:
        ...
        for item in tests:
            ...
            if (
                thistest != None
                and not thistest.snake
                and not thistest.food
                and not visited[thistest.xcoord][thistest.ycoord]
            ):
                if (
                    nexttest != None
                    and not nexttest.snake
                    and not nexttest.food
                    and not visited[nexttest.xcoord][nexttest.ycoord]
                ):
